...
**Reclaiming expired leases:** Each step, P's `get_finished()` checks **every** outstanding lease rather than stopping at the first unexpired one. Heartbeats extend leases in place, and in bidirectional mode `kv_lease_duration` and `decoder_kv_blocks_ttl` deadlines share the same map, so the leases are not ordered by expiry.
