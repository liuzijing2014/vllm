...
class CompressedTensorsMoEMethod(FusedMoEMethodBase):
    ...
    def get_moe_method(
        ...
        if quant_config._is_wNa16_group_channel(weight_quant, input_quant):
            ...
            if (
                (layer.local_num_experts >= 16 or layer.params_dtype != torch.float16)
...
class CompressedTensorsWNA16MoEMethod(CompressedTensorsMoEMethod):
    ...
    def apply(
        ...
        if dump:
            torch.save(
                topk_weights,
                f"{DUMP_DIR}/moe/compressed_tensors_topk_weights_tp_{tp_size}_r{rank}.pt",
            )
            torch.save(
                topk_ids,
                f"{DUMP_DIR}/moe/compressed_tensors_topk_ids_tp_{tp_size}_r{rank}.pt",
            )
