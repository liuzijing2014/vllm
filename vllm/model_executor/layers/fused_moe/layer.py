...
class FusedMoE(torch.nn.Module):
    ...
    def forward(
        ...
        if log:
            print(f"{self.use_direct_call=}")
        if self.use_direct_call:
            return self.forward_impl(hidden_states, router_logits, dump, log)
        else:
            return torch.ops.vllm.moe_forward(
                hidden_states, router_logits, self.layer_name, dump, log
            )
    ...
    def forward_impl(
        ...
        if log:
            ...
            print(f"{self.ep_size=}")
            print(f"{self.tp_size=}")
        ...
        if dump:
            torch.save(
                hidden_states,
                f"{DUMP_DIR}/moe/moe_impl_hidden_states_tp_{self.tp_size}_r{rank}.pt",
            )
            torch.save(
                router_logits,
                f"{DUMP_DIR}/moe/moe_impl_router_logits_tp_{self.tp_size}_r{rank}.pt",
            )
        # Matrix multiply.
        ...
        if dump:
            torch.save(
                final_hidden_states,
                f"{DUMP_DIR}/moe/moe_impl_inter_hidden_states_tp_{self.tp_size}_r{rank}.pt",
            )
        ...
        if self.dp_size > 1:
            ...
            if log:
                print(f"Before dp allReduce {final_hidden_states.shape=}")

            all_hidden_states = get_dp_group().all_reduce(final_hidden_states)
            if log:
                print(f"During dp allReduce {all_hidden_states.shape=}")
            final_hidden_states = all_hidden_states[start:end, :]
            if log:
                print(f"After dp allReduce {final_hidden_states.shape=}")

        if self.reduce_results and (self.tp_size > 1 or self.ep_size > 1):
            if log:
                print(f"Before tp allReduce {final_hidden_states.shape=}")
            # Default set to False. (May have to add shared expert outputs.)
            final_hidden_states = tensor_model_parallel_all_reduce(final_hidden_states)
            if log:
                print(f"After tp allReduce {final_hidden_states.shape=}")

        if dump:
            torch.save(
                final_hidden_states,
                f"{DUMP_DIR}/moe/moe_impl_final_hidden_states_tp_{self.tp_size}_r{rank}.pt",
            )
